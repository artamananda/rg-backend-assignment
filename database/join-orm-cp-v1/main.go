...
func (t Teacher) Join(db *gorm.DB) ([]Joined, error) {
	results := []Joined{}
	result := db.Model(&t).Select("? as teacher_name, schools.name as school_name, classes.name as class_name, lessons.name as lesson_name", t.Name).Joins("left join schools on schools.id = ?", t.SchoolID).Joins("left join classes on classes.id = ?", t.ClassID).Joins("left join lessons on lessons.id = ?", t.LessonID).Scan(&results)
